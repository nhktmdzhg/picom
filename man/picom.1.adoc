...
[[shadow-color]]*--shadow-color* _STRING_::
...
*--root-pixmap-shader* _SHADER_::
	Specify GLSL fragment shader path for rendering the root pixmap. The root pixmap usually is your desktop wallpaper, though for some desktop environment this is not the case. Shader is searched first relative to the directory the configuration file is in, then in the usual places for a configuration file. See section xref:_shader_interface[*SHADER INTERFACE*] below for more details on the interface.
...
Within each sub-item, these keys are available: ::
  ...
  shadow-color:::
	Color of the shadow. Same syntax as <<shadow-color>>.
  ...
  shader:::
	GLSL fragment shader specification for rendering window contents. See section xref:_shader_interface[*SHADER INTERFACE*] below for more details on the interface. This option can be a simple path to the shader file, but it's also possible to specify extra options to the shader. Currently, the only extra options supported are macro defines. Syntax:
+
----
shader = {
...
----
+
This will prepend `#define COLOR vec4(1.0, 0.0, 0.0, 1.0)` to your shader (note the lack of double quotes). The value of a define MUST be a string, any string will be accepted and no sanitization is performed.
...
Supported predefined targets are: :::
  ...
  `urgent`::::
	Whether the window is expecting immediate user action, as defined by the ICCCM. This only checks if the urgency bit is set in _WM_HINTS_. Interpretation of this bit varies among applications and window managers. Some of them also sets _pass:[_]NET_WM_STATE_DEMANDS_ATTENTION_ in _pass:[_]NET_WM_STATE_ to indicate urgency, so you might want to also check that (e.g. `_NET_WM_STATE@[*] = '_NET_WM_STATE_DEMANDS_ATTENTION'`).
...
=== Common options
  ...
  _triggers_:::
	...
	Valid triggers are: ::::
	  ...
	  _size_, _position_:: When the size or position of a window is changed. If both changed, the size trigger has priority. (EXPERIMENTAL)
...
_Shader effects (glx/egl only)_:::
+
A set of GLSL fragment shader effects ported from https://github.com/Schneegans/Burn-My-Windows[Burn-My-Windows].
They apply a custom shader to the window while the animation runs, and are only
available with the *glx* and *egl* backends. These shaders are compiled into
the picom binary, so the preset names work out of the box; to customize the
effect parameters, copy the corresponding `bmw-<nick>.frag` file (shipped in
the source tree under `data/shaders/`), edit the constants inside, and point
the _shader_ option of your own animation script at your copy. Note that the
Burn-My-Windows effects are licensed under the GNU GPL, version 3 or later.
+
Each effect is available as an `-in` variant for the open trigger and an `-out`
variant for the close trigger. The options are: ::
+
--
  _duration_:: Duration of the animation in seconds. Defaults are:
  fire 1.5s, glide 0.25s, glitch 0.75s, tv 0.4s, tv-glitch 0.75s,
  pixelate 0.5s, pixel-wheel 0.3s, pixel-wipe 1s, wisps 1.5s, portal 1.25s,
  hexagon 1.5s, energize-a 2s, energize-b 1s, aura-glow 0.75s, focus 0.5s,
  rgbwarp 0.75s.
--
+
...
+
--
  _fire_:: The window is consumed by flames, the classic burning-window effect.
  _glide_:: The window squishes, tilts and fades away.
  _glitch_:: Digital RGB-split glitch blocks.
  _tv_:: Classic CRT power-off collapse to a line.
  _tv-glitch_:: CRT turn-off with digital glitching.
  _pixelate_:: Pixelated dissolve with noise.
  _pixel-wheel_:: Pixelated rotating wheel wipe.
  _pixel-wipe_:: Pixelated wipe expanding from the center of the window.
  _wisps_:: Wispy curl-noise smoke dissolve.
  _portal_:: Swirling orange portal.
  _hexagon_:: Hexagonal grid mask dissolve with glow.
  _energize-a_:: Beam-down / energize column fade.
  _energize-b_:: Electric particle shower dissolve.
  _aura-glow_:: Glowing blurred aura expanding from the window edges.
  _focus_:: Progressive blur and edge dissolve.
  _rgbwarp_:: Per-channel RGB warp.
--
...
These options are valid for a timing function: :::
  ...
  _curve_:: Type of the curve and its parameters. It can be _linear_, which takes no parameters and defines a linear curve; or _cubic-bezier_, which takes four parameters for the four control points of the cubic bezier curve; or _steps_, which takes one or two parameters, the first is the number of steps, the second is the "jumpterm", which can be _jump-start_, _jump-end_, _jump-none_, or _jump-both_. The _curve_ option is optional, is not specified, the linear curve will be used.
...
Currently, these output variables are supported: :::
  ...
  _shadow-offset-x_, _shadow-offset-y_:: The offset of the shadow in the X and Y direction, respectively. The shadow will be moved by this amount. This offset applies *after* the window and the shadow has been scaled.
...
WARNING: The _saved-image-blend_ variable is experimental. It might work incorrectly, cause visual artifacts, or slow down your system. You are welcome to open an issue on GitHub if you encounter any problems to help us improve it, though resolution is not guaranteed.
  ...
  _shader-progress_:: While the window is running an animation that has a
  _shader_ option (see below), this number (between 0 and 1) is passed to the
  shader as the x component of the *progress* uniform. It has no other effect
  on the window.

  _shader-duration_:: Similar to _shader-progress_, this number is passed to
  the shader as the y component of the *progress* uniform, in seconds. Use a
  negative value to signal an opening animation to the shader.

  In addition to the output variables above, animation scripts support one
  extra common option: *shader*:: A GLSL fragment shader specification (see
  xref:_shader_interface[*SHADER INTERFACE*]), either a plain path or a group
  with _path_ and optional _defines_. While the animation is running, the
  window is rendered with this shader. The shader receives a vec2 uniform
  named *progress* (at uniform location 26): x is the value of
  _shader-progress_, y is the value of _shader-duration_. The names of the
  built-in Burn-My-Windows shaders (e.g. `bmw-fire.frag`) are resolved from
  the shaders compiled into the binary, so they work without any files on
  disk; a shader file of the same name next to the configuration file (or in
  the `shaders` directory of the XDG configuration locations) overrides the
  built-in one. Shaders ported from Burn-My-Windows are licensed under the
  GNU GPL, version 3 or later. Custom shaders are only supported by the
  *glx* and *egl* backends; on other backends the shader is ignored and the
  animation has no visible effect.
