// Ported to picom from Burn-My-Windows.
// Original effect: resources/shaders/fire.frag
// SPDX-FileCopyrightText: Simon Schneegans <code@simonschneegans.de>
// SPDX-License-Identifier: GPL-3.0-or-later
//
// Effect-specific uniforms from Burn-My-Windows are hardcoded to their default
// values below. To customize them, copy this file, edit the constants, and
// point the `shader` option of an animation script at your copy.

// =============================================================================
// picom port preamble
//
// This file is compiled as its own GLSL shader object by picom's window
// shader pipeline, linked together with picom's blit/masking shaders (see
// src/backend/gl/shaders.c). It declares the inputs it uses, defines
// `vec4 window_shader()`, and is multiplied by the mask factor by picom.
//
// The window texture `tex` uses premultiplied alpha; texcoord is in pixels.
// `bmw_progress` is filled per-frame by the animation system:
//   x = raw animation progress, 0 to 1 for both opening and closing
//       animations (exactly like uProgress in Burn-My-Windows),
//   y = +animation duration for closing animations, -duration for opening
//       animations. uForOpening below derives the direction from the sign.
#version 330
#extension GL_ARB_explicit_uniform_location : enable

in vec2 texcoord;
uniform sampler2D tex;
layout(location = 26) uniform vec2 bmw_progress;

// Map the Burn-My-Windows standard uniforms onto picom's interface.
#define uProgress bmw_progress.x
#define uDuration abs(bmw_progress.y)
#define uForOpening (bmw_progress.y < 0.0)
#define uIsFullscreen false
#define uPadding 0.0
#define uSize vec2(textureSize(tex, 0))
#define iTexCoord (texcoord / uSize)
...
void setOutputColor(vec4 outColor) { bmw_out = outColor; }
...
// ==== effect body ====
const bool u3DNoise = false;
const float uScale = 1.0;
const float uMovementSpeed = 0.5;
const vec4 uGradient1 = vec4(0.298, 0.2, 0.098, 0.0);
const vec4 uGradient2 = vec4(0.706, 0.216, 0.118, 0.7);
const vec4 uGradient3 = vec4(1.0, 0.298, 0.149, 0.9);
const vec4 uGradient4 = vec4(1.0, 0.651, 0.098, 1.0);
const vec4 uGradient5 = vec4(1.0, 1.0, 1.0, 1.0);
const bool uRandomColor = false;
const float uSeed = 1234.0;
...
vec4 window_shader() {
  bmw_out = vec4(0.0);
  bmw_main();
  return vec4(bmw_out.rgb * bmw_out.a, bmw_out.a);
}
