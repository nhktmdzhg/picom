...
void codegen(const char *name, const char *body, const struct placeholder *placeholders,
	...
	printf("static bool win_script_preset__%s(struct win_script *output, "
	       "config_setting_t *setting, const char *include_dir) {\n",
	       ident);
	printf("    output->script = script_template__%s(output->output_indices);\n", ident);
	if (shader_path != NULL) {
		printf("    output->shader = win_script_preset_shader(include_dir, "
		       "\"%s\");\n",
		       shader_path);
		printf("    if (output->shader == NULL) {\n");
...
/// Escape a string as a C string literal. Returns a malloc'ed string including
/// the surrounding quotes.
static char *c_string_escape(const char *src) {
	size_t len = strlen(src);
	// Worst case: every byte becomes an octal escape + NUL.
...
/// A simple growable string buffer.
struct charbuf {
	char *data;
	size_t len;
	size_t cap;
...
static void charbuf_put(struct charbuf *buf, const char *fmt, ...) {
	va_list ap;
	va_start(ap, fmt);
	int needed = vsnprintf(NULL, 0, fmt, ap);
	va_end(ap);
	BUG_ON(needed < 0);
	if (buf->len + (size_t)needed + 1 > buf->cap) {
		buf->cap = buf->len + (size_t)needed + 1 + 4096;
...
/// Write `buf` to `path`, atomically. If the existing file content is
/// identical, the file is left untouched (preserving its mtime, so that
/// build systems don't needlessly rebuild dependents).
static int charbuf_write_if_changed(const struct charbuf *buf, const char *path) {
	...
	if (f) {
		...
		while (true) {
			...
			if (n > 0) {
				...
				if (read_total > buf->len ||
				    memcmp(chunk, buf->data + read_total - n, n) != 0) {
					identical = false;
					break;
				}
			}
		...
		fclose(f);
		if (identical && read_total == buf->len) {
			return 0;
		}
	}

	size_t tmp_path_len = strlen(path) + 5;
	char *tmp_path = malloc(tmp_path_len);
	BUG_ON(!tmp_path);
	snprintf(tmp_path, tmp_path_len, "%s.tmp", path);
	f = fopen(tmp_path, "wb");
	if (!f) {
	...
	if (fwrite(buf->data, 1, buf->len, f) != buf->len || fclose(f) != 0) {
...
/// animgen shaders <dir> [output]: read every `.frag` file in `dir` and emit
/// a C file with the shader sources embedded, for building them into the
/// binary. If `output` is NULL, the output is written to stdout.
static int run_shaders_mode(const char *dir, const char *output) {
	DIR *d = opendir(dir);
	if (!d) {
		fprintf(stderr, "Failed to open shader directory %s\n", dir);
		return 1;
	}

	char **names = dynarr_new(char *, 16);
	struct dirent *entry;
	while ((entry = readdir(d)) != NULL) {
		size_t name_len = strlen(entry->d_name);
		if (name_len < 6 || strcmp(entry->d_name + name_len - 5, ".frag") != 0) {
			continue;
		}
		dynarr_push(names, strdup(entry->d_name));
	}
	closedir(d);

	if (dynarr_len(names) == 0) {
		fprintf(stderr, "No .frag shader files found in %s\n", dir);
		dynarr_free_pod(names);
		return 1;
	}
	qsort(names, dynarr_len(names), sizeof(char *), cmp_str);

	struct charbuf buf = {};
	charbuf_put(&buf, "// This file is generated by tools/animgen.c. DO NOT EDIT!\n");
	charbuf_put(&buf, "//\n");
	charbuf_put(&buf, "// SPDX-FileCopyrightText: Simon Schneegans "
	                  "<code@simonschneegans.de>\n");
	charbuf_put(&buf, "// SPDX-License-Identifier: GPL-3.0-or-later\n");
	charbuf_put(&buf, "//\n");
	charbuf_put(&buf, "// Shader sources ported from Burn-My-Windows, embedded into "
	                  "the\n");
	charbuf_put(&buf, "// binary. The Burn-My-Windows effects are licensed under "
	                  "the\n");
	charbuf_put(&buf, "// GNU GPL, version 3 or later.\n");
	charbuf_put(&buf, "#include <stddef.h>\n");
	charbuf_put(&buf, "#include <stdlib.h>\n");
	charbuf_put(&buf, "#include <string.h>\n\n");
	charbuf_put(&buf, "#include \"transition/bmw_shaders.h\"\n\n");
	charbuf_put(&buf, "const struct bmw_shader_entry bmw_shader_sources[] = {\n");
	dynarr_foreach(names, name) {
		...
		char *escaped = c_string_escape(src);
		charbuf_put(&buf, "    {\"%s\",\n     %s},\n", *name, escaped);
		free(escaped);
		free(src);
		free(path);
	...
	charbuf_put(&buf, "};\n");
	charbuf_put(&buf, "const unsigned number_of_bmw_shader_sources = %zu;\n",
	            dynarr_len(names));
	charbuf_put(&buf, "\n");
	// The table is sorted by path, so we can use bsearch for the lookup.
	charbuf_put(&buf, "static int bmw_shader_entry_cmp(const void *key, const void "
	                  "*elem) {\n");
	charbuf_put(&buf, "	return strcmp((const char *)key, ((const struct "
	                  "bmw_shader_entry *)elem)->path);\n");
	charbuf_put(&buf, "}\n");
	charbuf_put(&buf, "const char *bmw_lookup_embedded_shader(const char *path) {\n");
	charbuf_put(&buf, "	const struct bmw_shader_entry *entry = bsearch(\n");
	charbuf_put(&buf, "	    path, bmw_shader_sources, "
	                  "number_of_bmw_shader_sources,\n");
	charbuf_put(&buf, "	    sizeof(bmw_shader_sources[0]), "
	                  "bmw_shader_entry_cmp);\n");
	charbuf_put(&buf, "	return entry ? entry->source : NULL;\n");
	charbuf_put(&buf, "}\n");

	int ret;
	if (output) {
		ret = charbuf_write_if_changed(&buf, output);
	} else {
...
int main(int argc, const char **argv) {
	if (argc == 3 && strcmp(argv[1], "shaders") == 0) {
		return run_shaders_mode(argv[2], NULL);
	}
	if (argc == 4 && strcmp(argv[1], "shaders") == 0) {
		return run_shaders_mode(argv[2], argv[3]);
	}
	if (argc != 2) {
		return 1;
	}
	...
	auto conf_basename = strrchr(argv[1], '/');
	conf_basename = conf_basename ? conf_basename + 1 : argv[1];
	...
	for (unsigned i = 0; i < (unsigned)config_setting_length(settings); i++) {
		...
		auto shader_setting = config_setting_get_member(sub, "shader");
		if (shader_setting) {
			auto tmp = config_setting_get_string(shader_setting);
			if (!tmp) {
				fprintf(stderr,
				        "The \"shader\" option in preset %s must be "
				        "a string.\n",
				        name);
				return 1;
			}
			// The setting is removed below, which frees its value, so
			// we need to make a copy first.
			shader_path = strdup(tmp);
			config_setting_remove(sub, "shader");
		}
		...
		if (!has_err) {
			...
			codegen(name, code, placeholders, shader_path);
			free(code);

			dynarr_push(presets, strdup(name));
		}
		free((void *)shader_path);
