...
/// animgen shaders <dir>: read every `.frag` file in `dir` and emit a C file
/// with the shader sources embedded, for building them into the binary.
static int run_shaders_mode(const char *dir) {
	DIR *d = opendir(dir);
	if (!d) {
	...
	printf("// This file is generated by tools/animgen.c. DO NOT EDIT!\n");
	printf("//\n");
	printf("// SPDX-FileCopyrightText: Simon Schneegans <code@simonschneegans.de>\n");
	printf("// SPDX-License-Identifier: GPL-3.0-or-later\n");
	printf("//\n");
	printf("// Shader sources ported from Burn-My-Windows, embedded into the\n");
	printf("// binary. The Burn-My-Windows effects are licensed under the\n");
	...
	printf("};\n");
	printf("const unsigned number_of_bmw_shader_sources = %zu;\n", dynarr_len(names));
	printf("\n");
	printf("const char *bmw_lookup_embedded_shader(const char *path) {\n");
	printf("	for (unsigned i = 0; i < number_of_bmw_shader_sources; i++) {\n");
	printf("		if (strcmp(bmw_shader_sources[i].path, path) == 0) {\n");
	printf("			return bmw_shader_sources[i].source;\n");
	printf("		}\n");
	printf("	}\n");
	printf("	return NULL;\n");
	printf("}\n");
...
int main(int argc, const char **argv) {
	if (argc == 3 && strcmp(argv[1], "shaders") == 0) {
		return run_shaders_mode(argv[2]);
	}
	if (argc != 2) {
		return 1;
	}
